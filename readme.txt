...
Server:
    Our server program attempts to accept a connection from a client program through a socket descriptor. Upon receiving the socket descriptor, the parent process, which is the bank, forks, creating a child process that handles the communication between the client and the bank. We apply locks to the bank during searches for accounts, which take place in openAccount(), startAccount(), and printBankInfo().
